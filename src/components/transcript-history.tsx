        ...
        {finalSegments.length === 0 ? (
          ...
          finalSegments.map((segment, index) => (
            <article key={segment.id} className="rounded-xl border border-slate-200/80 bg-white/80 p-2.5 shadow-sm dark:border-slate-700 dark:bg-slate-950/50">
              ...
              <p className="text-xs font-medium leading-5 text-slate-500 dark:text-slate-400">{segment.text}</p>
              {segment.translatedText ? (
                <p className="mt-2 border-t border-slate-200/70 pt-2 text-sm font-semibold leading-6 text-brand-700 dark:border-slate-700 dark:text-cyan-200">
                  {segment.translatedText}
                </p>
              ) : null}
            </article>
